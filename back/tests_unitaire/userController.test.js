...
// Tests pour la fonction deleteUser
  describe('deleteUser', () => {
    ...
    it('Doit supprimmer un user', async () => {
      ...
      expect(res.send).toHaveBeenCalledWith({ message: 'Utilisateur supprimé avec succès' });
    });
...
// Tests pour la fonction Signup
  describe('signup', () => {
    ...
    // it('Devrait gérer les erreur d/email dupliqués', async () => {
    //   // Setup
    //   req.body = {
    //     username: 'testuser',
...
// Tests pour la fonction Login
  describe('login', () => {
    it('Devrait se connecter avec succès et renvoyer un token', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Connexion réussie', token: 'token' });
    });
    ...
    it('devrait renvoyer 400 si l/utilisateur est introuvable', async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.send).toHaveBeenCalledWith({ message: 'Identifiants incorrects' });
    });
