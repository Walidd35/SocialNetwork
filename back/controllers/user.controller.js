...
exports.signup = async (req, res) => {
  try {
    ...
    if (req.body.roles) {
      ...
      res.send({
        message: `Utilisateur ${username} enregistré avec succès avec le rôle spécifié.`,
        roles,
      });
    } else {
      ...
      if (!defaultRole) {
        return res
          .status(403)
          .send({ message: "Le rôle par défaut n'existe pas." });
      }
      await user.setRoles([defaultRole.role_id]);
      res.send({
        message: `Utilisateur ${username} enregistré avec succès avec le rôle par défaut.`,
      });
    }
  } catch (err) {
    if (err.name === "SequelizeUniqueConstraintError") {
      ...
      res
        .status(500)
        .send({ message: "Une erreur est survenue lors de l'inscription." });
    }
  }
};
...
exports.updateUser = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ message: "Utilisateur mis à jour avec succès", user });
  } catch (error) {
